...
Every file and directory has a unique absolute path in the file system, 
    ...
    file/directory itself, all concatenated by '/'s. 
...
Example 4:
    ...
    Since the absolute path for anything at the root directory is 
        just the name itself, the answer is "longfile.txt" with length 12.

Constraints:
    1 <= input.length <= 104
    input may contain: 
        lowercase or uppercase English letters, 
...
let lengthLongestPath1 = (input) => {
    let directory = input.split('\n');
    let max = 0, length = [], depth = 0;
    for (let x = 0; x < directory.length; x++) {
        depth = directory[x].lastIndexOf('\t') + 1;
        if (directory[x].includes('.')) {
            let fileLength = directory[x].length - depth;
            max = Math.max((length[depth-1] + 1 || 0) + fileLength, max);
        } 
        else length[depth] = directory[x].length - depth + (length[depth-1] + 1 || 0);
    }
    return max;
}
...
let lengthLongestPath = (input) => {
    const map = {};
    let maxLen = 0;
    for(let line of input.split('\n')) {
        const depth = (line.match(/\t/g) || []).length;
        console.log(`line & depth:  ${line}, ${depth}`);
        map[depth] = line.length - depth + (map[depth-1] || 0);
        console.log(`map after mapping:  ${JSON.stringify(map)}`);
