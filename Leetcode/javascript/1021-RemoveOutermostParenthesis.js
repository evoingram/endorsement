...
A valid parentheses string s is primitive if it is nonempty, and 
    there does not exist a way to split it into s = A + B, 
...
removeOuterParens = (stringToCheck) => {
    let newString = "";
    let storage = [];
    let primals = new Set();

    storage.push(stringToCheck[0]);
    ...
    for (let x = 0; x < stringToCheck.length; x++) {
        if(primals.has(x)) continue
        else newString += stringToCheck[x]
    }
